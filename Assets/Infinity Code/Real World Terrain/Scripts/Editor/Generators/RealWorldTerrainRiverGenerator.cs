...
namespace InfinityCode.RealWorldTerrain.Generators
{
    public static class RealWorldTerrainRiverGenerator
    {
        ...
        public static void CreateRAMRiverArea(RealWorldTerrainOSMBase osm, GameObject container, List<Vector3> points)
        {
            ...
            for (int i = 0; i < points.Count; i++) points[i] -= cPos;

            lakePolygon.maximumTriangleSize = 10000;
            lakePolygon.yOffset = 0;
            lakePolygon.snapToTerrain = true;
        ...
        public static void CreateRAMRiverStream(RealWorldTerrainOSMWay way, GameObject container, RealWorldTerrainContainer globalContainer)
        {
            ...
            Vector3 cPos = container.transform.position;
            foreach (Vector3 p in points) spline.AddPoint(p - cPos);

            spline.GenerateSpline();
            spline.snapToTerrain = true;
        ...
        private static void CreateRiver(RealWorldTerrainOSMBase osm, GameObject container, Material defMaterial, List<Vector3> points)
        {
            ...
            int[] triangles = RealWorldTerrainMath.Triangulate(flatPoints).ToArray();
            bool reversed = RealWorldTerrainMath.IsClockWise(vertices[triangles[0]], vertices[triangles[1]], vertices[triangles[2]]);
        ...
        public static void Generate(RealWorldTerrainContainer baseContainer)
        {
            ...
            string containerName = RealWorldTerrainWindow.generateTarget != null? "Rivers for " + RealWorldTerrainWindow.generateTarget.gameObject.name: "Rivers";
            GameObject container = RealWorldTerrainUtils.CreateGameObject(baseContainer, containerName);
